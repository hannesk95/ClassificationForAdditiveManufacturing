...
##HVD-Step1: import horovod
import horovod.tensorflow.keras as hvd
###HVD-Step1.

##HVD-Step2: Initialize horovod and configure GPUs
hvd.init()
...
##HVD-Step3: wrap your optimizer within hvd.DistributedOptimizer()
opt = hvd.DistributedOptimizer(opt)
##HVD-Step3.

##HVD-Step4: Create a callback that broadcast the model in GPU 0 to all the GPUs (reason: the mirroring strategy requires all GPUs holding the same model)
...
##HVD-Step5: Bare in mind that an epoch is performed by several GPUs in coordination. Therefore, in total each GPU does less steps to achive an epoch
batch_size = 32
#global_batch_size = batch_size  * 8 
steps = len(x_train) // batch_size // 8
###HVD-Step5.

##HVD-Step6: be sure you indicate the steps per epoch in the fit function as well as the callback
hist = model.fit(x_train, y_train, steps_per_epoch=steps, callbacks=callbacks, batch_size=batch_size, epochs=100, validation_data=(x_test, y_test), verbose=1 if hvd.local_rank()==0 else 0, shuffle=True)
##HVD-Step6.
